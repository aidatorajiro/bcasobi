...
class MerkleTree {
   ...
   * @param {String[]} leaves - Array of hashed leaves. Each leaf must be a String.
  ...
  /**
   * getRoot
   * @desc Returns the Merkle root hash as a String.
   * @return {String}
   * @example
   ...
   * @param {String} leaf - Target leaf
   * @param {Number} [index] - Target leaf index in leaves array.
   * Use if there are leaves containing duplicate data in order to distinguish it.
   * @return {String[]} - Array of String hashes.
   * @example
   ...
   * to the Merkle root.
   * @param {String[]} proof - Array of proof String hashes that should connect
   * target node to Merkle root.
   * @param {String} targetNode - Target node String
   * @param {String} root - Merkle root String
   * @return {Boolean}
   * @example
   * const root = tree.getRoot()
   * const proof = tree.getProof(leaves[2])
  ...
  verify(proof, targetNode, root) {
    ...
    for (let i = 0; i < proof.length; i++) {
      ...
      hash = this.hashAlgo(buffers.join(""))
    }
